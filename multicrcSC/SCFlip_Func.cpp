...
void crcEncode(vector<bool> &CRCInformation, vector<int> CRC_LENGTHS, vector<int> crcPolynomial, vector<int> INFO_LENGTHS)
{
    int segNum = CRC_LENGTHS.size();
    int start = 0;
    for (int i = 0; i < segNum; i++)
    {
        long int dividend = 0;

        for (int j = start; j < start + CRC_LENGTHS[i]; j++)
        {
            dividend <<= 1;
            dividend += CRCInformation[j];
        }

        for (int j = start + CRC_LENGTHS[i]; j < start + CRC_LENGTHS[i] + INFO_LENGTHS[i]; j++)
        {
            dividend <<= 1;
            dividend += CRCInformation[j];
            if ((dividend >> CRC_LENGTHS[i]) == 0)
                continue;
            dividend ^= crcPolynomial[i];
        }

        for (int j = 0; j < CRC_LENGTHS[i]; j++)
        {
            CRCInformation[start + INFO_LENGTHS[i] + j] = (dividend >> (CRC_LENGTHS[i] - 1 - j)) & 1;
        }

        start += INFO_LENGTHS[i] + CRC_LENGTHS[i];
    }
}
...
void generateChanellLLR(int CODE_LENGTH, double EbN0dB, vector<double> &ChannelLLR)
{
    FILE *fp;
    char filename[256];
    sprintf(filename, "./LLR/%.2lfLen%d.txt", EbN0dB, CODE_LENGTH);
...
void generateInformationArray(vector<bool> &Information, vector<bool> &CRCInformation, vector<int> CRC_LENGTHS, vector<int> INFO_LENGTHS)
{
    int segNum = CRC_LENGTHS.size();
    for (int i = 0; i < segNum; i++)
    {
        for (int j = 0; j < INFO_LENGTHS[i]; j++)
        {
            ...
            CRCInformation.push_back(a);
        }
        for (int j = 0; j < CRC_LENGTHS[i]; j++)
        {
            CRCInformation.push_back(0);
        }
    }
}
...
void initializeCRC(vector<int> CRC_LENGTHS, vector<int> &crcPolynomial)
{
    int segNum = CRC_LENGTHS.size();
    for (int i = 0; i < segNum; i++)
    {
        ...
        {
            crcPolynomial[i] = 0x1;
            crcPolynomial[i] = (crcPolynomial[i] << 1) + 1;
        }
        else if (CRC_LENGTHS[i] == 4)
        {
            crcPolynomial[i] = 0x9;
            crcPolynomial[i] = (crcPolynomial[i] << 1) + 1;
        }
        else if (CRC_LENGTHS[i] == 5)
        {
            crcPolynomial[i] = 0x1A;
            crcPolynomial[i] = (crcPolynomial[i] << 1) + 1;
        }
        else if (CRC_LENGTHS[i] == 8)
        {
            crcPolynomial[i] = 0xA6;
            crcPolynomial[i] = (crcPolynomial[i] << 1) + 1;
        }
        else if (CRC_LENGTHS[i] == 10)
        {
            crcPolynomial[i] = 0x327;
            crcPolynomial[i] = (crcPolynomial[i] << 1) + 1;
        }
        else if (CRC_LENGTHS[i] == 16)
        {
            crcPolynomial[i] = 0x8810;
            crcPolynomial[i] = (crcPolynomial[i] << 1) + 1;
        }
        else if (CRC_LENGTHS[i] == 0)
        {
            crcPolynomial[i] = 0;
        }
        else
